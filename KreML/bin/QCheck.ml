...
let ident = 
  let helper = fix (fun self _  ->
  let letter_or_underscore =
    frequency [1, char_range '_' '_'; 1, char_range 'A' 'Z'; 8, char_range 'a' 'z';] in
...
let const = frequency [4, map (fun i -> Const_int i) nat; 1, map (fun b -> Const_bool b) bool]
...
let typ = sized @@ fix (fun self n ->
  let s = self @@ n / 64 in
  match n with
  | 0 -> frequency [1, return Typ_bool; 1, return Typ_int; 1, return Typ_unit;
   (* 2, map (fun id -> Typ_var id) small_nat *)
   ]
  | _ -> frequency
...
let pattern = sized @@ fix (fun self n -> match n with
| 0 -> frequency [
  5, map (fun id -> Pat_var id) ident;
  1, return Pat_nil; 1, return Pat_unit; 1, return Pat_wildcard;
  2, map (fun c -> Pat_const c) const]
| n -> frequency [
  2, map (fun (fst, snd, rest) -> Pat_tuple(fst, snd, rest)) (tuple @@ self @@ n/2);
  2, map2 (fun x xs -> Pat_cons(x, xs)) (self @@ n / 64) (self @@ n / 64);
  1, map2 (fun p t -> Pat_constrained(p, t)) (self @@ n / 64) typ]
 )

let expr = sized @@ fix (fun self n ->
  let s = self @@ n / 64 in
  match n with
  | 0 -> frequency [2, map (fun c -> Expr_const c) const;
    2, map (fun id -> Expr_var id) ident;
    1, return Expr_unit; 1, return Expr_nil]
  | _ -> frequency [
    1, map2 (fun x xs -> Expr_cons (x, xs)) s s;
    1, map (fun (fst, snd, rest) -> Expr_tuple(fst, snd, rest)) (tuple @@ s);
    3, (fun rf pat value scope -> Expr_let(rf, (pat, value), scope)) 
        <$> rec_flag
        <*> pattern
        <*> s
        <*> s;
    2, map2 (fun p body -> Expr_fun(p, body)) pattern s;
    1, map2 (fun e cases -> Expr_match(e, cases))
       s 
...
let arbitrary_pattern =
  ...
  | Pat_constrained(p, _) -> shrink p in
  let print = show_pattern in
  QCheck.make ~print ~shrink pattern


let check_pattern =
  let property = check_ast_component
   ~parser:(Kreml_lib.Parser.typed_pattern)
   ~to_code:(Kreml_lib.Ast_printer.pattern_to_code)
   ~to_ast:(Format.asprintf "%a" pp_pattern) in
  QCheck.Test.make arbitrary_pattern property ~count:100 ~name:"pattern = parse @@ to_code @@ pattern";;
...
let arbitrary_expr =
  ...
  let rec shrink = function |
    ...
    | Expr_constrained(e, _) -> return e in
...
let check_expr =
  let property = check_ast_component
    ...
    ~to_ast:(Format.asprintf "%a" pp_expr) in
  QCheck.Test.make arbitrary_expr property ~count:100 ~name:"expr = parse @@ to code @@ expr";;
...
let check_structure =
  let property = check_ast_component
    ...
    ~to_ast:(Format.asprintf "%a" pp_structure) in
  QCheck.Test.make arbitary_structure property ~count:10 ~name:"structure = parse @@ pp @@ structure";;
