...
module Alpha_transformer = struct
  ...
  | Pat_tuple(fst, snd, rest) ->
    ...
    let* rest', ctx = List.fold_right (fun elem acc ->
  ...
  | Expr_tuple(fst, snd, rest) ->
    ...
    let* rest' = List.fold_right (fun elem acc ->
      let* acc = acc in
  ...
  | Expr_constrained(e, _) -> transform_expr ctx e
  | Expr_ite(c, t, e) ->
  ...
  let transform_structure s =
    ...
    let transform_struct_item ctx (Str_value(rf, bindings)) =
      let* ctx = List.fold_left (fun ctx (p, _) ->
        let* ctx = ctx in 
        let* _, ctx = transform_pattern id_gen ctx p in
        return ctx)
      ctx
      bindings in      
      let* bindings', ctx = List.fold_right (fun (p, e) acc ->
        let* acc, ctx = acc in
        let* _, ctx = transform_pattern id_gen ctx p in
        let* e' = transform_expr ctx e in
        ((p, e')::acc, ctx) |> return)
      bindings
      (return ([], ctx)) in
      (Str_value(rf, bindings'), ctx) |> return in
