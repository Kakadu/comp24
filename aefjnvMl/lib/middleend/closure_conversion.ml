...
let expr_vars = 
  let pat_set p= 
    Base.List.fold_left (pattern_vars p) ~f:(fun acc p -> Base.Set.add acc p) ~init:empty_set
  in
  let rec helper acc = function
    ...
    | Exp_function (p, e) ->
      Base.Set.diff (helper acc e) (pat_set p)
    | Exp_let (d, e) ->
      let acc = helper acc e in
      helper_decl acc d
    | Exp_match (e, cases) ->
      let acc = Base.List.fold_left cases ~f:(fun acc (p, e) -> Base.Set.diff (helper acc e) (pat_set p)) ~init:acc in
      helper acc e
    | Exp_ifthenelse(e, e', e'') ->
      helper (helper (helper acc e) e') e''
  ...
  and helper_decl acc = function
    ...
    | Decl(Recursive, bindings) ->
      let acc = Base.List.fold_left bindings ~f:(fun acc {vb_pat=_; vb_expr} -> helper acc vb_expr) ~init:acc in
      Base.List.fold_left bindings ~f:(fun acc {vb_pat; vb_expr=_} -> Base.Set.diff acc (pat_set vb_pat)) ~init:acc
    | _ -> acc
  in
...
let closure_conversion toplvl = 
  let rec closure_expr env = function
    ...
    | Exp_function (p, e) ->
      ...
      let saturated = Base.Set.fold free_vars ~f:(fun acc v -> (pvar v) :: acc) ~init:(p::ps) in
  ...
  and closure_decl env = 
    let helper rec_flag env = function
      | {vb_pat = Pat_var v as pv; vb_expr} ->  
        ...
        let free_vars = Base.Set.diff free_vars (set toplvl) in
        let free_vars = match rec_flag with Recursive -> Base.Set.remove free_vars v | Nonrecursive -> free_vars in
        let saturated = Base.Set.fold free_vars ~f:(fun acc v -> (pvar v) :: acc) ~init:ps in
    ...
    | Decl (Recursive, bindings) -> 
      let ps, _ = Base.List.fold_left bindings ~init:([], []) ~f:(fun (ps, es) {vb_pat; vb_expr} -> (vb_pat::ps, vb_expr::es)) in
      let p_vars = Base.List.concat_map ps ~f:pattern_vars in
      let env = Base.List.fold_left p_vars ~init:env ~f:(fun acc pv -> Base.Map.set acc ~key:pv ~data:empty_set) in
      let env, bindings = Base.List.fold_left bindings ~init:(env, []) ~f:(fun (env, bindings) b -> let b, env = helper Recursive env b in env, b :: bindings) in
      let env', bindings' =  Base.List.fold_left bindings ~init:(env, []) ~f:(fun (env, bindings) b -> let b, env = helper Recursive env b in env, b :: bindings) in
...
let convert_program ast = 
  let decls, _ = Base.List.fold_left ast ~init:([], std_lib_names) ~f:(fun (ss, tlvls) s -> 
    let s, tlvls = closure_conversion tlvls s in (s::ss, tlvls))
  in
  Base.List.rev decls