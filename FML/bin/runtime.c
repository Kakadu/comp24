...
closure_t *create_closure(int64_t fun_ptr, int64_t args_num,
                          int64_t args_applied) {
  size_t size = sizeof(closure_t) + args_num * sizeof(int64_t);
  closure_t *clos = (closure_t *)malloc(size);
  clos->fun_ptr = fun_ptr;
  clos->args_num = args_num;
  clos->args_applied = args_applied;
  return clos;
}

closure_t *empty_closure(int64_t fun_ptr, int64_t args_num) {
  return create_closure(fun_ptr, args_num, 0);
}

int64_t _copy_closure(closure_t *src_clos, int64_t new_args_num,
                              va_list *new_args) {
  int64_t total_args_applied = src_clos->args_applied + new_args_num;
  closure_t *new_clos =
      create_closure(src_clos->fun_ptr, src_clos->args_num, total_args_applied);
...
int64_t call_closure(closure_t *closure, int64_t new_args_num,
  ...
  if (ffi_prep_cif(&cif, FFI_DEFAULT_ABI, args_count, &ffi_type_sint64,
                   arg_types) == FFI_OK) {
    ffi_call(&cif, (void *)closure->fun_ptr, &res, (void **)args);
  } else {
...
int64_t _apply_args_to_closure(closure_t *closure, int64_t new_args_num,
  ...
  if (args_num_until_apply <= new_args_num) {
    ...
    return _copy_closure(closure, new_args_num, new_args);
  }
}

int64_t apply_args(int64_t closure_ptr, int64_t new_args_num, ...) {
  va_list new_args;
