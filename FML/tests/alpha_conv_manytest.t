  ...
  $ ./alpha_conv_runner.exe < manytests/typed/002fac.ml
  ...
  $ ./alpha_conv_runner.exe < manytests/typed/003fib.ml
  ...
  $ ./alpha_conv_runner.exe < manytests/typed/004manyargs.ml
  ...
  $ ./alpha_conv_runner.exe < manytests/typed/005fix.ml
  ...
  $ ./alpha_conv_runner.exe < manytests/typed/006partial.ml
  let foo = (fun b -> if b
  then (fun a0 -> ((( + ) a0) 2))
  else (fun a1 -> ((( * ) a1) 10)))
  ...
  $ ./alpha_conv_runner.exe < manytests/typed/006partial2.ml
  ...
  let () = (print_int a2) in
  0
  $ ./alpha_conv_runner.exe < manytests/typed/006partial3.ml
  ...
  0
  $ ./alpha_conv_runner.exe < manytests/typed/007order.ml
  ...
  $ ./alpha_conv_runner.exe < manytests/typed/008ascription.ml
  ...
  $ ./alpha_conv_runner.exe < manytests/typed/012fibcps.ml
  let rec fib = (fun n k -> if ((( < ) n) 2)
  then (k n)
  else ((fib ((( - ) n) 1)) (fun a -> ((fib ((( - ) n) 2)) (fun b -> (k ((( + ) a) b)))))))
  
  let main = (print_int ((fib 6) (fun x -> x)))
  $ ./alpha_conv_runner.exe < manytests/typed/013foldfoldr.ml
  let id = (fun x -> x)
  ...
  $ ./alpha_conv_runner.exe < manytests/typed/015tuples.ml
  let rec fix = (fun f x -> ((f (fix f)) x))
  ...
  let fixpoly = (fun l -> ((fix (fun self a0 -> ((map (fun li x -> ((li (self a0)) x))) a0))) l))
  ...
  $ ./alpha_conv_runner.exe < manytests/typed/016lists.ml
  ...
  $ ./alpha_conv_runner.exe < manytests/do_not_type/001.ml
  Infer error:
  $ ./alpha_conv_runner.exe < manytests/do_not_type/002if.ml
  Infer error:
  $ ./alpha_conv_runner.exe < manytests/do_not_type/003occurs.ml
  Infer error:

  $ ./alpha_conv_runner.exe < manytests/do_not_type/004let_poly.ml
  Infer error:

  $ ./alpha_conv_runner.exe < manytests/do_not_type/015tuples.ml
  Infer error:
