...
let%expect_test _ =
  lambda_lifting_program {| let _ = let (x, y) = (1, 2) and (z :: _) = (1 :: [2]) in x + y + z |};
  [%expect {|
    let _ = (let (z :: _) = (1 :: (2 :: [])) and (x, y) = (1, 2) in (( + ) (( + ) x y) z)) |}]
;;

let%expect_test _ =
  lambda_lifting_program {| let _ = let (x, y) = (1, 2) and (z :: _) = (1 :: [2]) and f x = x*x*x in f (x + y + z) |};
  [%expect {|
    let ll_0 x = (( * ) (( * ) x x) x)
...
let%expect_test _ =
  lambda_lifting_program {| let _ = let rec fac x = if x = 1 then x else x * fac (x-1) in fac 1 |};
  [%expect {|
...
let%expect_test _ =
  lambda_lifting_program {| let _ = let rec f x = g x and g x = (fun _ -> f x) () in (f, g) |};
  [%expect {|
...
let%expect_test _ =
  lambda_lifting_program {| let _ = let rec f x = g x and g x = f x and x = 5 in (f, g, x) |};
  [%expect {|
...
let%expect_test _ =
  lambda_lifting_program {| let f = (fun x -> x) 0 and g x = (fun _ -> x) ();; let h x y = x + ((fun x -> x * x) y) |};
  [%expect {|
...
let%expect_test _ =
  lambda_lifting_program {| let f = if (fun x -> x) true then (fun y -> y) 0 else (fun () -> 0) () |};
  [%expect {|
    let ll_0 x = x
    let ll_1 y = y
    let ll_2 () = 0
