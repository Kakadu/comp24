...
let%expect_test _ =
  ...
  [%expect
    {| ((fun cc_1 cc_0 x -> (( + ) (( + ) x cc_0) cc_1)) z y) |}]
;;

let%expect_test _ =
  closure_conversion_expression {| fun x -> fun arg_0 -> y |};
  [%expect
    {| ((fun cc_0 x arg_0 -> cc_0) y) |}]
;;
...
let%expect_test _ =
  ...
  [%expect
    {| (((fun cc_0 x -> (( + ) x cc_0)) y) y) |}]
;;

let%expect_test _ =
  closure_conversion_expression {| (fun x -> x) (fun x -> y) (fun _ -> z) |};
  [%expect
    {| ((fun x -> x) ((fun cc_0 x -> cc_0) y) ((fun cc_1 _ -> cc_1) z)) |}]
;;
...
let%expect_test _ =
  closure_conversion_expression {| let f arg_0 = let g y = x + y + arg_0 + arg_1 in g 0 in f 0 |};
  [%expect
...
let%expect_test _ =
  ...
  [%expect
    {| (let cc_0 cc_1 x = (cc_1 x) in (cc_0 f)) |}]
;;

let%expect_test _ =
  closure_conversion_expression {| fun f -> let f x = f x in f |};
  [%expect
    {| (fun f -> (let cc_0 cc_1 x = (cc_1 x) in (cc_0 f))) |}]
;;
...
let%expect_test _ =
  ...
  [%expect
    {| (let f x = x and g x = x in ((f 0), (g 0))) |}]
;;

let%expect_test _ =
  closure_conversion_expression {| let f x = x and g x = x + y in (f 0, g 0) |};
  [%expect
    {| (let f x = x and g cc_0 x = (( + ) x cc_0) in ((f 0), ((g y) 0))) |}]
;;
...
let%expect_test _ =
  ...
  [%expect
    {| (let cc_0 x = x and g cc_1 x = (cc_1 x) in ((cc_0 0), ((g f) 0))) |}]
;;
...
let%expect_test _ =
  ...
  [%expect
    {| (let (x, y) = (1, 2) and g k = k in (x, y, (g 1))) |}]
;;
...
let%expect_test _ =
  closure_conversion_expression {| let (x, y) = (1, 2) and g k = k + x + y in (x, y, g 1) |};
  [%expect
...
let%expect_test _ =
  closure_conversion_expression {| let (x, arg_0) = (1, 2) and g k = k + x + y in (x, arg_0, g 1) |};
  [%expect
...
let%expect_test _ =
  ...
  [%expect
    {| (let rec f cc_0 x = ((f cc_0) cc_0) in (f y)) |}]
;;

let%expect_test _ =
  closure_conversion_expression {| let rec f x = if x = 1 then x else x * f (x - 1) in f 5 |};
  [%expect
    {| (let rec f x = (if (( = ) x 1) then x else (( * ) x (f (( - ) x 1)))) in (f 5)) |}]
;;

let%expect_test _ =
  closure_conversion_expression {| let rec f _ = if x = 1 then x else x * f (x - 1) in f 5 |};
  [%expect
...
let%expect_test _ =
  ...
  [%expect
    {| (let rec f x = (f x) and g x = (g x) in (f, g)) |}]
;;
...
let%expect_test _ =
  ...
  [%expect
    {| (let rec f x = (g x) and g x = (f x) in (f, g)) |}]
;;
...
let%expect_test _ =
  closure_conversion_expression {| if x > 0 then (fun x -> x + y) x else (fun x y -> x + y) x x |};
  [%expect
    {| (if (( > ) x 0) then (((fun cc_0 x -> (( + ) x cc_0)) y) x) else ((fun x y -> (( + ) x y)) x x)) |}]
;;

let%expect_test _ =
  closure_conversion_expression {| if ((fun x -> x + k) x) > 0 then (fun x -> x + y) x else (fun x y -> x + y) x x |};
  [%expect
...
let%expect_test _ =
  ...
  [%expect
    {| ((fun cc_0 x -> (if (( > ) x cc_0) then x else cc_0)) y) |}]
;;
...
let%expect_test _ =
  ...
  [%expect
    {| (match x with (m, n) -> (( + ) m n) | _ -> 0) |}]
;;

let%expect_test _ =
  closure_conversion_expression {| match x with | (m, n) -> (fun _ -> m + n) () | _ -> 0 |};
  [%expect
    {| (match x with (m, n) -> (((fun cc_1 cc_0 _ -> (( + ) cc_0 cc_1)) n m) ()) | _ -> 0) |}]
;;

let%expect_test _ =
  closure_conversion_expression {| fun x -> match x with | (m, n) -> (fun _ -> m + n) () | _ -> 0 |};
  [%expect
    {| (fun x -> (match x with (m, n) -> (((fun cc_1 cc_0 _ -> (( + ) cc_0 cc_1)) n m) ()) | _ -> 0)) |}]
;;

let%expect_test _ =
  closure_conversion_expression {| fun x -> match x with | (m, n) -> (fun _ -> m + n + k) () | _ -> 0 |};
  [%expect
    {| ((fun cc_0 x -> (match x with (m, n) -> (((fun cc_3 cc_2 cc_1 _ -> (( + ) (( + ) cc_2 cc_3) cc_1)) n m cc_0) ()) | _ -> 0)) k) |}]
;;

let%expect_test _ =
  closure_conversion_expression {| fun x -> (match ((fun x -> x + y) 1) with 5 -> 0 | _ -> (fun x -> x + y + 1) 1) |};
  [%expect
    {| ((fun cc_0 x -> (match (((fun cc_1 x -> (( + ) x cc_1)) cc_0) 1) with 5 -> 0 | _ -> (((fun cc_2 x -> (( + ) (( + ) x cc_2) 1)) cc_0) 1))) y) |}]
;;

let%expect_test _ =
  closure_conversion_expression {| fun x -> (match ((fun x -> x + y) 1) with 5 -> 0 | _ -> (fun x -> x + z + 1) 1) |};
  [%expect
...
let%expect_test _ =
  ...
  [%expect
    {| (fun cc_0 -> (match cc_0 with (m, n) -> (( + ) m n) | _ -> 0)) |}]
;;
...
let%expect_test _ =
  closure_conversion_expression {| fun x -> function | (m, n) -> (fun _ -> m + n) () | _ -> x |};
  [%expect
    {| (fun x -> ((fun cc_3 cc_0 -> (match cc_0 with (m, n) -> (((fun cc_2 cc_1 _ -> (( + ) cc_1 cc_2)) n m) ()) | _ -> cc_3)) x)) |}]
;;

let%expect_test _ =
  closure_conversion_expression {| fun x -> function | (m, n) -> (fun _ -> m + n + k) () | _ -> x + k |};
  [%expect
    {| ((fun cc_0 x -> ((fun cc_6 cc_5 cc_1 -> (match cc_1 with (m, n) -> (((fun cc_4 cc_3 cc_2 _ -> (( + ) (( + ) cc_3 cc_4) cc_2)) n m cc_5) ()) | _ -> (( + ) cc_6 cc_5))) x cc_0)) k) |}]
;;

let%expect_test _ =
  closure_conversion_expression {| fun x -> function | (m, n) -> (fun _ -> m + n + k) () | _ -> x + z |};
  [%expect
...
let%expect_test _ =
  ...
  [%expect
    {| ((fun (x : int) -> (x : int)) : (int -> int)) |}]
;;

let%expect_test _ =
  closure_conversion_expression {| ((fun (x : int) -> (y : int)) : int -> int) |};
  [%expect
    {| (((fun cc_0 (x : int) -> (cc_0 : int)) y) : (int -> int)) |}]
;;
