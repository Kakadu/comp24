...
let rec pp_type_definition ppf = function
  ...
  | TDArrow (t1, t2) -> Format.fprintf ppf "(%a -> %a)" pp_type_definition t1 pp_type_definition t2
  | TDTuple (t1, t2, ts) ->
    Format.fprintf ppf "(%a)"
      (Format.pp_print_list ~pp_sep:(fun ppf () -> Format.fprintf ppf " * ") pp_type_definition)
      (t1 :: t2 :: ts)
...
let rec pp_pattern ppf = function
  ...
  | PTuple (p1, p2, ps) ->
    Format.fprintf ppf "(%a)"
      ...
      (p1 :: p2 :: ps)
  | PListConstructor (p1, p2) -> Format.fprintf ppf "(%a :: %a)" pp_pattern p1 pp_pattern p2
...
let rec pp_expression ppf = function
  ...
  | EFun ((p, ps), e) ->
    Format.fprintf ppf "(fun %a -> %a)"
      (Format.pp_print_list ~pp_sep:(fun ppf () -> Format.fprintf ppf " ") pp_pattern)
      (p :: ps) pp_expression e
  | EFunction (c, cs) ->
    Format.fprintf ppf "(function %a)"
      (Format.pp_print_list ~pp_sep:(fun ppf () -> Format.fprintf ppf " | ") pp_match_case) (c :: cs)
  | EApplication (e1, e2, es) ->
    Format.fprintf ppf "(%a)"
      (Format.pp_print_list ~pp_sep:(fun ppf () -> Format.fprintf ppf " ") pp_expression) (e1 :: e2 :: es)
  | EIfThenElse (cond, t, f) ->
    Format.fprintf ppf "(if %a then %a%a)" pp_expression cond pp_expression t
      (fun ppf -> function
         | None -> ()
  ...
  | ETuple (e1, e2, es) ->
    Format.fprintf ppf "(%a)"
      (Format.pp_print_list ~pp_sep:(fun ppf () -> Format.fprintf ppf ", ") pp_expression) (e1 :: e2 :: es)
  | EListConstructor (e1, e2) -> Format.fprintf ppf "(%a :: %a)" pp_expression e1 pp_expression e2
  | EEmptyList -> Format.fprintf ppf "[]"
  | EMatchWith (e, c, cs) ->
    Format.fprintf ppf "(match %a with %a)" pp_expression e
      (Format.pp_print_list ~pp_sep:(fun ppf () -> Format.fprintf ppf " | ") pp_match_case) (c :: cs)
  | ELetIn (c, cs, e) ->
    Format.fprintf ppf "(let %a in %a)"
      (Format.pp_print_list ~pp_sep:(fun ppf () -> Format.fprintf ppf " and ") pp_decl_case) (c :: cs)
      pp_expression e
  | ERecLetIn (c, cs, e) ->
    Format.fprintf ppf "(let rec %a in %a)"
      (Format.pp_print_list ~pp_sep:(fun ppf () -> Format.fprintf ppf " and ") pp_decl_case) (c :: cs)
      pp_expression e
...
and pp_decl_case ppf (p, e) =
  ...
  | EFun _ ->
    ...
    Format.fprintf ppf "%a %a = %a"
      pp_pattern p
      (Format.pp_print_list ~pp_sep:(fun ppf () -> Format.fprintf ppf " ") pp_pattern) args
      pp_expression body
...
let pp_declaration ppf = function
  | DOrdinary (c, cs) ->
    Format.fprintf ppf "let %a"
      (Format.pp_print_list ~pp_sep:(fun ppf () -> Format.fprintf ppf " and ") pp_decl_case) (c :: cs)
  | DRecursive (c, cs) ->
    Format.fprintf ppf "let rec %a"
      (Format.pp_print_list ~pp_sep:(fun ppf () -> Format.fprintf ppf " and ") pp_decl_case) (c :: cs)

let pp_program ppf p =
  Format.pp_print_list ~pp_sep:(fun ppf () -> Format.fprintf ppf "\n") pp_declaration ppf p
;;
