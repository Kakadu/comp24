...
and ll_expr =
  ...
  | LELetIn of ll_case * ll_case list * ll_expr
  (** Let in (without fun): 'let f = 1 and g = 2 in f + g *)
  | LEIfThenElse of ll_expr * ll_expr * ll_expr option
  ...
  | LEMatchWith of ll_expr * ll_case * ll_case list
  (** Pattern matching: match x with | y -> y *)
  | LETyped of ll_expr * type_definition
  (** Assigning a type to an expression: (expr : int) *)
[@@deriving show { with_path = false }]

(** Binding mainPattern * patterns * expression *)
type ll_decl_case = pattern * pattern list * ll_expr

and ll_decl =
  | LDOrdinary of ll_decl_case * ll_decl_case list
  (** Top-level let-binding: 'let f x = x and g x = x' *)
