...
let eliminate_pm_fun env args expr =
      ...
      List.fold_left
        (fun acc arg ->
          ...
          | _ ->
            ...
            return
              (env, fresh_id :: processed_args, fresh_id :: match_args, arg :: patterns))
        (return (env, [], [], []))
        args
    in
  ...
  | [ single_match_arg ] ->
    ...
    let case_expr =
      eliminate_pm_case (pat, to_match) (new_body, pattern_matching_failure_flag)
    in
    return ((processed_args, case_expr), env)
  | _ ->
    (* Сombining complex patterns into one. *)
    let new_pat =
      PTuple (List.hd patterns, List.nth patterns 1, List.tl (List.tl patterns))
    in
    let to_match =
    ...
    let case_expr =
      eliminate_pm_case
        (new_pat, PMFIdentifier fresh_id)
        (new_body, pattern_matching_failure_flag)
    in
