           ...
           let update_pattern pattern =
             let rec aux pattern acc_replacement_map =
               ...
               | PVar (Id name) when IdentifierSet.mem (Id name) patterns_to_update ->
                 ...
                 return (PVar (Id new_name), IdentifierMap.add (Id name) (Id new_name) acc_replacement_map)
               | PTuple (p1, p2, ps) ->
                 ...
                 let* new_ps, final_map =
                   List.fold_left
                     (fun acc p ->
                        ...
                        return (new_p :: acc_ps, IdentifierMap.union (fun _ v _ -> Some v) acc_map new_map))
                     (return ([], map2))
                     ps
                 in
