...
let rec closure_expression env fv_map = function
  ...
  | EFun _ as expr -> 
    let* fun_closure, _ = closure_fun true env fv_map (FunctionTransformer.transform_fun expr) in
    return fun_closure
...
and closure_fun application_flag env fv_map = function
  | EFun ((p, ps), body) as expr ->
    ...
    let old_free_vars = 
      IdentifierSet.fold 
        (fun var acc -> (EIdentifier var) :: acc) 
        free_vars
        []
    in
...
and closure_let_in env fv_map (case, cases) expr =
  ...
  let* transformed_decl = FunctionTransformer.transform_let_in env_for_decl_transform (ELetIn (case, cases, expr)) in
...
and closure_rec_let_in env fv_map (case, cases) expr previous_replacement_maps =
  ...
  let* closed_cases, current_replacement_maps =
    List.fold_left
      (fun acc (p, expr) ->
         ...
         match p, expr with
         | PVar i, EFun _ -> 
           let* closed_expr, replacement_map = closure_fun false extended_env updated_fv_map expr in
           return ((p, closed_expr) :: acc_cases, IdentifierMap.add i replacement_map acc_replacement_map)
         | _ ->
  ...
  in
  let closed_cases = List.rev closed_cases in
  let closed_cases = apply_replacement_map_to_cases previous_replacement_maps closed_cases in
  let closed_cases = apply_replacement_map_to_cases current_replacement_maps closed_cases in
  let* closed_body = closure_expression env updated_fv_map expr in

  let case_contains_fv_predicate = 
    fun (_, expr) -> not (IdentifierSet.is_empty (IdentifierSet.diff (get_expr_free_vars expr) extended_env)) 
  in
  if List.exists case_contains_fv_predicate cases then
    closure_rec_let_in env fv_map (List.hd closed_cases, List.tl closed_cases) closed_body current_replacement_maps
  else
    return @@ ERecLetIn (List.hd closed_cases, List.tl closed_cases, closed_body)
;;
