...
let declaration_alpha_conversion env replacement_map = function
  | PMFDOrdinary (name, args, body) ->
    let* name', env, replacement_map' =
      generate_new_name_if_needed name env replacement_map
    in
    let* args', env, replacement_map'' =
      List.fold_right
        (fun arg_name acc ->
          let* acc, env, replacement_map = acc in
          let* new_arg_name, env, replacement_map =
            generate_new_name_if_needed arg_name env replacement_map
          in
  ...
  | PMFDRecursive (case, cases) ->
    ...
    let bypass_func (name, args, body) acc =
      ...
      let* _, _, replacement_map_to_save =
        generate_new_name_if_needed name env replacement_map_to_save
      in
      let* name', env, replacement_map =
        generate_new_name_if_needed name env replacement_map
      in
      ...
      let* body' = expression_alpha_conversion env replacement_map body in
      return
      @@ ((name', args', body') :: acc, env, replacement_map, replacement_map_to_save)
    in
