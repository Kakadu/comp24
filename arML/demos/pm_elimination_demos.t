MANYTESTS
  ...
  let test3 a b c = let a = (print_int a) in
  let b = (print_int b) inlet c = (print_int c) in
  0
  ...
  let main = let foo = (foo 1) inlet foo = (foo 2) in
  let cc_0 = (foo 3) inlet () = (print_int cc_0) in
  0
  ...
  let map f p = let a = (unpack_tuple p 0) in
  ...
  let feven p n = let e = (unpack_tuple p 0) in
  let o = (unpack_tuple p 1) in
  (if (( = ) n 0) then 1 else (o (( - ) n 1)))
  
  let fodd p n = let e = (unpack_tuple p 0) in
  let o = (unpack_tuple p 1) in
  ...
  let rec ll_2 acc xs = (if (equal (get_list_length xs) 0) then acc else (if (less_than 0 (get_list_length xs)) then let h = (unpack_list_hd xs) in
  let tl = (unpack_list_tl xs) in
  (ll_2 (( + ) acc 1) tl) else pattern_matching_failure))
  
  let length_tail = (ll_2 0)
  
  let rec map f xs = (if (equal (get_list_length xs) 0) then [] else (if (logic_and (less_than 0 (get_list_length xs)) (equal (get_list_length (unpack_list_tl xs)) 0)) then let a = (unpack_list_hd xs) in
  ((f a) :: []) else (if (logic_and (less_than 1 (get_list_length xs)) (logic_and (less_than 0 (get_list_length (unpack_list_tl xs))) (equal (get_list_length (unpack_list_tl (unpack_list_tl xs))) 0))) then let a = (unpack_list_hd xs) in
  let b = (unpack_list_hd (unpack_list_tl xs)) in
  ((f a) :: ((f b) :: [])) else (if (logic_and (less_than 2 (get_list_length xs)) (logic_and (less_than 1 (get_list_length (unpack_list_tl xs))) (logic_and (less_than 0 (get_list_length (unpack_list_tl (unpack_list_tl xs)))) (equal (get_list_length (unpack_list_tl (unpack_list_tl (unpack_list_tl xs)))) 0)))) then let a = (unpack_list_hd xs) in
  let b = (unpack_list_hd (unpack_list_tl xs)) in
  let c = (unpack_list_hd (unpack_list_tl (unpack_list_tl xs))) in
  ((f a) :: ((f b) :: ((f c) :: []))) else (if (logic_and (less_than 3 (get_list_length xs)) (logic_and (less_than 2 (get_list_length (unpack_list_tl xs))) (logic_and (less_than 1 (get_list_length (unpack_list_tl (unpack_list_tl xs)))) (less_than 0 (get_list_length (unpack_list_tl (unpack_list_tl (unpack_list_tl xs)))))))) then let a = (unpack_list_hd xs) in
  let b = (unpack_list_hd (unpack_list_tl xs)) in
  let c = (unpack_list_hd (unpack_list_tl (unpack_list_tl xs))) in
  let d = (unpack_list_hd (unpack_list_tl (unpack_list_tl (unpack_list_tl xs)))) in
  let tl = (unpack_list_tl (unpack_list_tl (unpack_list_tl (unpack_list_tl xs)))) in
  ((f a) :: ((f b) :: ((f c) :: ((f d) :: (map f tl))))) else pattern_matching_failure)))))
  
  let rec append xs ys = (if (equal (get_list_length xs) 0) then ys else (if (less_than 0 (get_list_length xs)) then let x = (unpack_list_hd xs) in
  let xs = (unpack_list_tl xs) in
  (x :: (append xs ys)) else pattern_matching_failure))
  
  let rec ll_1 xs = (if (equal (get_list_length xs) 0) then [] else (if (less_than 0 (get_list_length xs)) then let h = (unpack_list_hd xs) in
  let tl = (unpack_list_tl xs) in
  (append h (ll_1 tl)) else pattern_matching_failure))
  
  let concat = ll_1
  
  let rec iter f xs = (if (equal (get_list_length xs) 0) then () else (if (less_than 0 (get_list_length xs)) then let h = (unpack_list_hd xs) in
  let tl = (unpack_list_tl xs) inlet () = (f h) in
  (iter f tl) else pattern_matching_failure))
  
  let rec cartesian xs ys = (if (equal (get_list_length xs) 0) then [] else (if (less_than 0 (get_list_length xs)) then let h = (unpack_list_hd xs) in
  let tl = (unpack_list_tl xs) in
  (append (map (ll_0 h) ys) (cartesian tl ys)) else pattern_matching_failure))
