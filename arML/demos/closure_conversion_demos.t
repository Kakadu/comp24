MANYTESTS
  $ ./start_closure_conversion_demos.exe < manytests/typed/001fac.ml
  let rec fac n = (if (( <= ) n 1) then 1 else (( * ) n (fac (( - ) n 1))))
  let main = (let () = (print_int (fac 4)) in 0)

  $ ./start_closure_conversion_demos.exe < manytests/typed/002fac.ml
  let rec fac_cps n k = (if (( = ) n 1) then (k 1) else (fac_cps (( - ) n 1) ((fun cc_1 cc_0 p -> (cc_0 (( * ) p cc_1))) n k)))
  let main = (let () = (print_int (fac_cps 4 (fun print_int -> print_int))) in 0)

  $ ./start_closure_conversion_demos.exe < manytests/typed/003fib.ml
  ...
  $ ./start_closure_conversion_demos.exe < manytests/typed/004manyargs.ml
  ...
  $ ./start_closure_conversion_demos.exe < manytests/typed/005fix.ml
  let rec fix f x = (f (fix f) x)
  let fac self n = (if (( <= ) n 1) then 1 else (( * ) n (self (( - ) n 1))))
  let main = (let () = (print_int (fix fac 6)) in 0)

  $ ./start_closure_conversion_demos.exe < manytests/typed/006partial.ml
  let foo b = (if b then (fun foo -> (( + ) foo 2)) else (fun foo -> (( * ) foo 10)))
  let foo x = (foo true (foo false (foo true (foo false x))))
  let main = (let () = (print_int (foo 11)) in 0)

  $ ./start_closure_conversion_demos.exe < manytests/typed/006partial2.ml
  ...
  $ ./start_closure_conversion_demos.exe < manytests/typed/006partial3.ml
  let foo a = (let () = (print_int a) in (fun b -> (let () = (print_int b) in (fun c -> (print_int c)))))
  let main = (let () = (foo 4 8 9) in 0)

  $ ./start_closure_conversion_demos.exe < manytests/typed/007order.ml
  ...
  $ ./start_closure_conversion_demos.exe < manytests/typed/008ascription.ml
  let addi f g x = ((f x ((g x) : bool)) : int)
  let main = (let () = (print_int (addi (fun x b -> (if b then (( + ) x 1) else (( * ) x 2))) (fun _start -> (( = ) (( / ) _start 2) 0)) 4)) in 0)

  $ ./start_closure_conversion_demos.exe < manytests/typed/009let_poly.ml
  let temp = (let f x = x in ((f 1), (f true)))

  $ ./start_closure_conversion_demos.exe < manytests/typed/015tuples.ml
  let rec fix f x = (f (fix f) x)
  let map f p = (let (a, b) = p in ((f a), (f b)))
  let fixpoly l = (fix (fun self l -> (map ((fun cc_1 cc_0 li x -> (li (cc_1 cc_0) x)) self l) l)) l)
  ...
  $ ./start_closure_conversion_demos.exe < manytests/typed/016lists.ml
  let rec length xs = (match xs with [] -> 0 | (h :: tl) -> (( + ) 1 (length tl)))
  let length_tail = (let rec helper acc xs = (match xs with [] -> acc | (h :: tl) -> (helper (( + ) acc 1) tl)) in (helper 0))
  ...
  let concat = (let rec helper xs = (match xs with [] -> [] | (h :: tl) -> (append h (helper tl))) in helper)
  let rec iter f xs = (match xs with [] -> () | (h :: tl) -> (let () = (f h) in (iter f tl)))
  let rec cartesian xs ys = (match xs with [] -> [] | (h :: tl) -> (append (map ((fun cc_0 a -> (cc_0, a)) h) ys) (cartesian tl ys)))
