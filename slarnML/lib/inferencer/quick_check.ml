...
let rec string_of_ty t =
  ...
  | TInt -> "int" | TBool -> "bool" | TUnit -> "unit"
...
let rec unify t1 t2 =
  ...
  | TVar ({ contents = Unbound (_, l1) } as v1),
    ...
    if v1 == v2 then () else
    if l1 < l2 then v2 := Link t1 else v1 := Link t2
  | TVar ({ contents = Unbound _ } as v), t
  | t, TVar ({ contents = Unbound _ } as v) ->
    if occurs_check v t then failwith "Occurs check failed"
    else v := Link t
  | TInt, TInt | TBool, TBool | TUnit, TUnit -> ()
  | TFun (a, b), TFun (c, d) ->
    unify a c;
    unify b d
  | _ -> failwith (Printf.sprintf "Type mismatch: %s vs %s" (string_of_ty t1) (string_of_ty t2))
...
let type_check init_ctx expr =
  let get_decl_name = function | Decl (name, _) | DeclRec (name, _) -> name in
  let get_decl_args = function | Decl (_, args) | DeclRec (_, args) -> args in
  let new_ctx ctx t = function | Decl _ -> ctx | DeclRec (x, _) -> (x, t)::ctx in
  (* let next_level_f d = if (List.length (get_decl_args d))=0 then (fun x->x) else (fun x->x+0) in *)
  let last_arg args = match List.rev args with 
  ...
  | Fun ([], body) -> let (t, _) = check ctx (level) body in (t, ctx)
  | Fun(arg::tail, body) -> 
  ...
  | Or(e1, e2) | And(e1, e2) | Eq(e1, e2) | Lt(e1, e2) | Gt(e1, e2) | Gte(e1, e2) | Lte(e1, e2) ->
  ...
  | If(e1, e2, e3) -> 
    ...
    unify e1_t TBool; unify e2_t e3_t;
    (e2_t, ctx)
  ...
  and instantiate level t =
    let t = deref t in
    match t with
    | TFun(a, r) -> TFun(instantiate level a, instantiate level r)
...
let quick_check ast_lst =
  (List.fold_left (fun res ast -> 
    res >>= (fun (types, ctx) -> 
      (type_check_res ctx ast) >>= (fun (t, ctx) -> 
        Result(types@[t], ctx)
  ))) (Result([], [])) ast_lst) >>= (fun (lst, _) -> Result lst)
;;
(* Тестирование *)
(* let () =
...
let () = 
  ...
  Let(Decl("fac2", ["n"]),
    LetIn(DeclRec("help2", ["n";"f"]),
      If(Lte(Id"n",Const(CInt 1)),
        App(Id"f", [Const(CInt 1)]), 
        App(Id"help2", [Sub(Id"n",Const(CInt 1)); Fun(["x"], Mul(App(Id"f", [Id"n"]), Id"x"))])),
  ...
  | Result list -> print_endline (String.concat "\n"
