...
| Ast.Id id -> (match List.find_opt (fun (name, _, _, _) -> name = id) env with
  ...
  | Some (_, new_name, _, _) -> CId new_name)
...
| Ast.Fun (args, body) ->
  let env_args = (List.map (fun arg -> (arg, arg, 0, [])) args) @ env in
  let body_converted = closure_conversion ~env:((List.hd args, List.hd args, 0, prt_args) :: env_args) ~prt_args:(args @ prt_args) body in
  CFun ((prt_args@args), body_converted)
| Ast.App (func, args) ->
  let func_converted = closure_conversion ~env func in
  let args_converted = List.map (closure_conversion ~env) args in
