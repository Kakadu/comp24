...
let is_keyword = function
  | "fun" | "let" | "in" | "if" | "then" | "else" | "true" | "false" | "not" | "rec" ->
    true
  | _ -> false
;;
...
let integer =
  ...
  let number sign =
    skip_empty *> take_while1 is_digit
    >>= fun num -> return (CInt (sign * int_of_string num))
  in
  (* (minus <|> plus) >>= number *)
...
let identifier =
  let word =
    skip_empty *> take_while1 (fun c -> Char.equal c '_' || is_digit c || is_letter c)
  in
  let checker word =
...
let tuple el =
  ...
  *> parens
       (lift2
          (fun lst el -> List.concat [ lst; [ el ] ])
          (many (el <* skip_empty <* char ','))
          (el <* skip_empty))
;;
...
let declaration is_rec =
  ...
  >>= fun id ->
  skip_empty *> arguments
  >>= fun arg_lst ->
  if is_rec then return (DeclRec (id, arg_lst)) else return (Decl (id, arg_lst))
;;
...
let compare_op =
  ...
  let neq =
    skip_empty *> (string "<>" <|> string "!=") *> return (fun e1 e2 -> Not (Eq (e1, e2)))
  in
...
let%test _ =
  parse_ok "qwertyuiopasdfghjklzxcvbnm1234567890_" "qwertyuiopasdfghjklzxcvbnm1234567890_"
;;
...
let%test _ =
  parse_ok " not(not (a) + 2)*b" (Mul (Not (Add (Not (Id "a"), Const (CInt 2))), Id "b"))
;;
  ...
  parse_ok
    ...
    (Mul
       ( Not
           (Add
              ( Not
                  (Add (Id "a", Sub (Div (Const (CInt 3), Const (CInt 2)), Const (CInt 1))))
              , Const (CInt 2) ))
       , Id "b" ))
;;
...
let%test _ =
  parse_ok
    " a + 2 = b * 3"
    (Eq (Add (Id "a", Const (CInt 2)), Mul (Id "b", Const (CInt 3))))
;;

let%test _ =
  parse_ok
    " a \n+ 2 > b * 3"
    (Gt (Add (Id "a", Const (CInt 2)), Mul (Id "b", Const (CInt 3))))
;;

let%test _ =
  parse_ok
    " a + 2\n >= b * 3"
    (Gte (Add (Id "a", Const (CInt 2)), Mul (Id "b", Const (CInt 3))))
;;

let%test _ =
  parse_ok
    " a + 2 != b * 3"
    (Not (Eq (Add (Id "a", Const (CInt 2)), Mul (Id "b", Const (CInt 3)))))
;;

let%test _ =
  parse_ok
    " a + 2 < b \t* 3"
    (Lt (Add (Id "a", Const (CInt 2)), Mul (Id "b", Const (CInt 3))))
;;

let%test _ =
  parse_ok
    " a + 2 \n\n<= b * 3"
    (Lte (Add (Id "a", Const (CInt 2)), Mul (Id "b", Const (CInt 3))))
;;

let%test _ =
  parse_ok
    " a < 2 && b = 3"
    (And (Lt (Id "a", Const (CInt 2)), Eq (Id "b", Const (CInt 3))))
;;

let%test _ =
  parse_ok
    " a < 2 || b = 3"
    (Or (Lt (Id "a", Const (CInt 2)), Eq (Id "b", Const (CInt 3))))
;;
...
let%test _ =
  parse_ok
    "a + (f 2 x) * 3"
    (Add (Id "a", Mul (App (Id "f", [ Const (CInt 2); Id "x" ]), Const (CInt 3))))
;;
  ...
  parse_ok
    ...
    (Add
       ...
       , Mul
           ( App
               ( Id "f"
               , [ Const (CInt 2); Id "x"; App (Id "g", [ Const (CInt 3); Id "y" ]) ] )
           , Const (CInt 3) ) ))
;;
       ...
       , Or
           ( Eq
               ( Add
                   ...
                   , App
                       ( Id "f"
                       , [ Const (CBool false); App (Id "g", [ Const (CInt 3); Id "y" ]) ]
                       ) )
               , Const (CInt 3) )
           , Const (CInt 2) ) ))
;;
...
let%test _ =
  parse_ok
    "let a = (b) let a = b;;"
    [ Let (Decl ("a", []), Id "b"); Let (Decl ("a", []), Id "b") ]
;;

let%test _ =
  parse_ok
    "let a = b;; let a c d = b"
    [ Let (Decl ("a", []), Id "b"); Let (Decl ("a", [ "c"; "d" ]), Id "b") ]
;;
...
let%test _ =
  parse_ok
    "let a b c = 1;; (a b c 1 (c + 2));; let b = (a c)"
...
let%test _ =
  parse_ok
    "let a = let b = 1 in let c = 2 in (d 3)"
    [ Let
        ( Decl ("a", [])
        , LetIn
            ( Decl ("b", [])
            , Const (CInt 1)
            , LetIn (Decl ("c", []), Const (CInt 2), App (Id "d", [ Const (CInt 3) ])) )
        )
    ]
;;
