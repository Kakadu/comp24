...
let rec pp_anf_aexpr tab ae = 
  ...
  let pp_anf_cexpr tab = function
    ...
    | AIf (e1, e2, e3) -> let next_tab = concat "" ["\t";tab] in
      concat "" ["if ("; pp_anf_immexpr e1; ")\n";
        tab;"then (\n";
        next_tab; pp_anf_aexpr next_tab e2; "\n";
        tab;") else (\n";
        next_tab; pp_anf_aexpr next_tab e3; ")"]
    | AApp (e, args) ->       concat "" ["("; pp_anf_immexpr e; " "; concat " " (List.map pp_anf_immexpr args); ")"]
  in
  match ae with
  | ALet (id, e1, e2) -> 
    concat "" ["(let ";id;"=";pp_anf_cexpr next_tab e1;"\n";tab;"in\n";tab;pp_anf_aexpr tab e2;")"]
  | ACExpr e -> pp_anf_cexpr tab e

let pp_anf_afun = function
  | AFun (id, args, e) -> concat "" ["(fun "; id; "("; concat " " args; ")->\n\t"; pp_anf_aexpr "\t" e; "\n)"]
;;