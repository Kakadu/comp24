...
let cc_ok n res expected = 
  ...
  | Result c_ast -> print_string@@String.concat "" [n;":\n";
...
let cc1 = 
  [CLet(Decl("fac", ["n"]),
    CLetIn(DeclRec("fack", ["n"; "k"]),
      CIf(CLte(CId"n",CConst(CInt(1))), 
        CApp(CApp(CId "n",[]), [CSub(CId"n", CConst(CInt 1))]), 
        CApp(CFun(["k";"n";"m"], CMul(CId"k", CMul(CId"m",CId"n"))), [CId "k"; CId "n"])),
...
let cc3 = 
  [CLet(Decl("f", ["a"; "b"]),
    CLetIn(Decl("g", ["b";"a";"c"]),
      CLetIn(Decl("h", ["c";"b";"a"]), 
        CApp(CFun(["c";"b";"a";"x"], CMul(CId"x", CApp(CApp(CId"a",[]),[CMul(CId"c",CId"b")]))),[CId"c";CId"b";CId"a"]),
...
let cc5 = 
  [CLet(Decl("f",["a"]),
    ...
    CLetIn(Decl("h",["a";"c"]),
      CMul(CId"a",CId"c"),
    CAdd(CApp(CApp(CId"h",[CId"a"]),[CConst(CInt 1)]), CApp(CApp(CId"g",[CId"a"]),[CConst(CInt 2)]))
    )))]
let%test _ = cc_ok "cc_5" (clos_conv ast5) cc5
;;
let ast6 = 
  [Let(Decl("f",["a"]),
    LetIn(Decl("g",[]), Fun(["x"],Id"x"),
      LetIn(Decl("h",[]), Fun(["x"],Mul(Id"a",Id"x")),
...
let cc6 = 
  [CLet(Decl("f",["a"]),
    ...
    CAdd(CApp(CApp(CId "g",[]), [CId"a"]), CApp(CApp(CId"h",[CId"a"]),[CId"a"]))
    )))]
...
let ll_ok n res expected = 
  ...
  | Result l_ast -> (print_string@@String.concat "" [n;":\n";
...
LFun("fack#fac",["n";"k"],LIf(LLte(LId"n",LConst(CInt(1))), 
...
;;
let ll2 = [
LFun("h#g#f",["a";"c";"d";"e"], LMul(LId"a",LAdd(LId"c",LMul(LId"d",LId"e"))));
LFun("g#f", ["a"; "c"; "d"], LApp("h#g#f",[LId"a";LId"c";LId"d";LConst(CInt 4)]));
LFun("f",["a"], LApp("g#f",[LId"a";LConst(CInt 2);LConst(CInt 3)]))
];;
ll_ok "ll_2" (lambda_lifting cc2) ll2
;;
let ll3 = [
LFun("anon$1#h#g#f",["c";"b";"a";"x"],LMul(LId"x", LApp("a",[LMul(LId"c",LId"b")])));
LFun("h#g#f",["c";"b";"a"], 
  LApp("anon$1#h#g#f",[LId"c";LId"b";LId"a"]));
...
LFun("g#f", ["a"; "b"], LApp("h#g#f",[LId"a";LConst(CInt 2);LConst(CInt 3)]));
...
LFun("f",["a"], LAdd(LApp("h#f",[LId"a";LConst(CInt 1)]), LApp("g#f",[LId"a";LConst(CInt 2)])))
];;
...
LFun("f",["a"], LAdd(LApp("g#f",[LId"a"]),LApp("h#f",[LId"a";LId"a"])))
];;
...
let anf_ok n ll expected = 
  ...
  | l_ast -> (print_string@@String.concat "" [n;":\n";
...
AFun("fac",["n"],
  ALet("anf_app#8",AApp(AId"anon$2#fac", []),
  ALet("anf_app#9",AApp(AId"fack#fac",[AId"n";AId"anf_app#8"]),
...
let anf4 = [
AFun("h#g#f",["a";"b";"c"],
  ALet("anf_op#1",ADiv(AId"b",AId"c"),
  ALet("anf_op#2",AMul(AId"a",AId"anf_op#1"),
  ACExpr(CImmExpr(AId"anf_op#2"))
  )));
AFun("g#f", ["a"; "b"],
  ALet("anf_app#3",AApp(AId"h#g#f",[AId"a";AInt 2;AInt 3]),
  ACExpr(CImmExpr(AId"anf_app#3"))
  ));
AFun("f",["a"],
  ALet("anf_op#4",AAdd(AInt 1,AInt 0),
  ALet("anf_app#5",AApp(AId"g#f",[AId"anf_op#4";AId"a"]),
...
let anf5 = [
AFun("g#f", ["a"; "b"],
  ALet("anf_op#1",ADiv(AId"a",AId"b"),
  ACExpr(CImmExpr(AId"anf_op#1"))
  ));
AFun("h#f",["a";"c"],
  ALet("anf_op#2",AMul(AId"a",AId"c"),
  ACExpr(CImmExpr(AId"anf_op#2"))
  ));
AFun("f",["a"],
  ALet("anf_app#3",AApp(AId"h#f",[AId"a";AInt 1]),
  ALet("anf_app#4",AApp(AId"g#f",[AId"a";AInt 2]),
  ALet("anf_op#5",AAdd(AId"anf_app#3",AId"anf_app#4"),
...
AFun("g#f",[],
  ALet("anf_app#1", AApp(AId"anon$1#g#f", []),
  ACExpr(CImmExpr(AId"anf_app#1"))));
AFun("anon$2#h#f",["a";"x"],
  ALet("anf_op#2",AMul(AId"a",AId"x"),
  ACExpr(CImmExpr(AId"anf_op#2"))));
AFun("h#f", ["a"],
  ALet("anf_app#3",AApp(AId"anon$2#h#f",[AId"a"]),
  ACExpr(CImmExpr(AId"anf_app#3"))));
AFun("f",["a"],
  ALet("anf_app#4",AApp(AId"g#f",[AId"a"]),
  ALet("anf_app#5",AApp(AId"h#f",[AId"a";AId "a"]),
  ALet("anf_op#6",AAdd(AId"anf_app#4",AId"anf_app#5"),
