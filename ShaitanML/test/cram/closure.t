  ...
  >   in
  >   fack n (fun x -> x)
  let fac = (fun n n -> let rec fack = (fun a0 k -> if ((( <= ) a0) 1)
  then (k 1)
  ...
  >   new_sum 5
  > EOF
  let sum = (fun x x -> let new_sum = (fun x y y -> ((( + ) x) y)) in
  (((new_sum x) y) 5))
  ...
  >   let z a = a (y + 1) in
  >   z (fun x -> x)
  let x = (fun y y -> let z = (fun a y a -> (a ((( + ) y) 1))) in
  (((z a) y) (fun a0 -> a0)))
  ...
  >       in
  >       fibo_cps n (fun x -> x)
  let fibo = (fun n n -> let rec fibo_cps = (fun a0 acc -> if ((( < ) a0) 3)
  then (acc 1)
