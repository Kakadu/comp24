...
module BoundVars = struct
  ...
  let merge old_map new_map =
    Base.Map.merge_skewed old_map new_map ~combine:(fun ~key:_ _ v -> v)
  ;;
end
...
let rec identify_free_vars bound_set = function
  ...
  | PEEApp (f, arg) ->
    StrSet.union (identify_free_vars bound_set f) (identify_free_vars bound_set arg)
  | PEELet (decl, next) -> handle_let_vars bound_set decl next
  | PEECons (hd, tl) ->
    StrSet.union (identify_free_vars bound_set hd) (identify_free_vars bound_set tl)
  | PEETuple exprs ->
    List.fold_left
      (fun acc e -> StrSet.union acc (identify_free_vars bound_set e))
      StrSet.empty
      exprs

and handle_let_vars bound_set decl next =
  match decl with
  | PENonrec (name, expr) ->
    StrSet.union
  ...
  | PERec bindings ->
    ...
    let body_vars =
      List.fold_left
        (fun acc (_, e) -> StrSet.union acc (identify_free_vars bound e))
        StrSet.empty
        bindings
    in
...
let rec convert_expr env bindings = function
  ...
  | PEEApp (f, arg) -> PEEApp (convert_expr env bindings f, convert_expr env bindings arg)
  | PEELet (decl, next) -> handle_let_conversion env bindings decl next
  | PEECons (hd, tl) ->
    PEECons (convert_expr env bindings hd, convert_expr env bindings tl)
  | PEETuple exprs -> PEETuple (List.map (convert_expr env bindings) exprs)

and handle_let_conversion env bindings decl next =
  match decl with
  | PENonrec (name, expr) ->
    let converted_expr, new_bindings =
      match expr with
      | PEEFun (params, body) ->
        let free =
          StrSet.diff (identify_free_vars env body) (StrSet.of_list params)
          |> StrSet.to_list
        in
        let converted = convert_expr env BoundVars.empty body in
    ...
    PEELet (PENonrec (name, converted_expr), convert_expr env merged next)
  | PERec decls ->
    let converted, new_bindings = convert_rec_decls env bindings decls in
    PEELet (PERec converted, convert_expr env (BoundVars.merge bindings new_bindings) next)

and convert_rec_decls env bindings decls =
  let names = List.map fst decls in
  let process_decl acc (name, expr) =
    match expr with
    | PEEFun (params, body) ->
    ...
    | _ -> [] :: fst acc, snd acc
  in
  let free_vars, bindings = List.fold_left process_decl ([], bindings) decls in
  ...
  let converted =
    List.map2
      (fun (name, expr) free ->
        ...
        | _ -> name, convert_expr env bindings expr)
      decls
      free_vars
  in
