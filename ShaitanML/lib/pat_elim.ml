...
let rec expr_to_ast = function
  ...
  | PEEIdent v -> EVar v
  | PEEEif (e1, e2, e3) -> EIf (expr_to_ast e1, expr_to_ast e2, expr_to_ast e3)
  | PEELet (decl, e) ->
    (match decl with
     | PENonrec (name, e1) ->
       ELet (Nonrec, (to_pattern name, expr_to_ast e1), expr_to_ast e)
     | PERec [ (name, e1) ] -> ELet (Rec, (to_pattern name, expr_to_ast e1), expr_to_ast e))
  | PEETuple e_list ->
...
let unpack_expr e = function
  | Tuple i -> PEEApp (PEEApp (PEEIdent "tuple_element", e), PEEConst (PECint i))
  | List_hd -> PEEApp (PEEIdent "list_head", e)
  | List_tl -> PEEApp (PEEIdent "list_tail", e)
  | Value -> e
;;
...
let unpack_pat_checks expr pat =
  ...
  let rec helper add_list cur = function
    ...
    | PConst c ->
      (match c with
       | CUnit -> []
       | _ -> [ PEEApp (PEEApp (PEEIdent "=", cur), PEEConst (convert_const c)) ])
    | PTuple pl ->
      let t = List.mapi pl ~f:(fun i p -> helper true (unpack_expr cur (Tuple i)) p) in
      List.concat t
    | PCons (l, r) ->
      let min_length = get_min_lenght 0 r in
      let list_length = PEEApp (PEEIdent "list_len", cur) in
      let check =
        PEEApp (PEEApp (PEEIdent ">", list_length), PEEConst (PECint min_length))
      in
...
let match_failure = PEEIdent "fail_match"
...
let rec rp_expr =
  let open Ast in
  function
  | EConstraint (e, _) -> rp_expr e
  | EConst c -> return @@ PEEConst (convert_const c)
  | EVar v -> return @@ PEEIdent v
  | EApply (e1, e2) ->
  ...
  | EFun (p, exp) ->
    ...
    (match List.length args_to_match with
     ...
     | 1 ->
       ...
       let to_match = PEEIdent (List.hd_exn args_to_match) in
     ...
     | _ ->
       ...
       let to_match =
         let vals = List.map args_to_match ~f:(fun a -> PEEIdent a) in
         PEETuple vals
       in
       let* fresh_name = fresh >>| get_name in
       let case_expr = create_case (PEEIdent fresh_name) pat new_body match_failure in
       return @@ PEEFun (new_args, PEELet (PENonrec (fresh_name, to_match), case_expr)))
  | EMatch (e_last, case_list) ->
    let* e = rp_expr e_last in
    (match e_last with
     | EVar _ | EConst _ -> rp_match e case_list
     | _ ->
       let* fresh_name = fresh >>| get_name in
       let* e_match = rp_match (PEEIdent fresh_name) case_list in
    ...
    (match pat with
     ...
     | _ ->
       (match e1 with
        ...
        | _ ->
          let* fresh_name = fresh >>| get_name in
          let case_expr = create_case (PEEIdent fresh_name) pat e2 match_failure in
          return @@ PEELet (PENonrec (fresh_name, e1), case_expr)))
  | ELet (Rec, (pat, e1), e2) ->
    let* decl = rp_rec_decl [ pat, e1 ] in
    let* e = rp_expr e2 in
...
let rp_toplevel = function
  | Ast.SValue (Nonrec, [ (pat, e) ]) ->
    ...
    (match pat with
     ...
     | pat ->
       let* fresh_name = fresh >>| get_name in
       let to_unpack = PEEIdent fresh_name in
       ...
       then return (PENonrec (fresh_name, e) :: decls)
         ...
         let ite = create_if checks (PEEConst PECUnit) (PEEIdent "fail_match") in
