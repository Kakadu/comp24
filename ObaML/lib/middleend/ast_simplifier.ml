...
let gen_pat_expr_name = "_gen_pat_expr"
let gen_tuple_getter_id = Ast.Id "_gen_tuple_getter"
...
let gen_list_getter_head_id = Ast.Id "_gen_list_getter_head"

let gen_list_getter_head lst =
  Simple_ast.SEApp (Simple_ast.SEVar gen_list_getter_head_id, lst)
;;

let gen_list_getter_tail_id = Ast.Id "_gen_list_getter_tail"
...
let construct_new_fun_expr pat gen_var_name curr_expr =
  let rec helper curr_gen_expr curr_expr = function
    ...
    | Ast.PVar var_name ->
      Simple_ast.SELet (Ast.Nonrecursive, var_name, curr_gen_expr, curr_expr)
    | Ast.PTuple tup_lst ->
      let new_expr, _ =
        List.fold_left
          (fun acc tup_pat ->
      ...
      new_expr
    | Ast.PCons (pat1, pat2) ->
      let h1 = helper (gen_list_getter_head curr_gen_expr) curr_expr pat1 in
      helper (gen_list_getter_tail curr_expr) h1 pat2
...
let construct_full_expr cases_conditions cases_expr =
  let rec helper = function
    ...
    | cond :: tl_cond, expr :: tl_expr ->
      Simple_ast.SEIf (cond, expr, helper (tl_cond, tl_expr))
    | _ -> Simple_ast.SEVar (Ast.Id "Hello proger")
  in
  helper (cases_conditions, cases_expr)
;;

let construct_equality_condition gen_decl_name case =
  let pat, _ = case in
  let rec helper current_condition current_gen_expr = function
    ...
    | Ast.PConst const ->
      ...
      in
      Simple_ast.SEApp
        ( Simple_ast.SEApp (Simple_ast.SEVar (Ast.Id "( && )"), current_condition)
        , new_condition )
    ...
    | Ast.PTuple pat_lst ->
      let new_cond, _ =
        List.fold_left
          (fun acc pat ->
            let cond, ind = acc in
            let new_cond = helper cond (gen_tuple_getter_fun ind current_gen_expr) pat in
            new_cond, ind + 1)
          (current_condition, 0)
          pat_lst
      in
    ...
    | Ast.PCons (pat1, pat2) ->
      let new_cond =
        helper current_condition (gen_list_getter_head current_gen_expr) pat1
      in
      helper new_cond (gen_list_getter_tail current_gen_expr) pat2
    | Ast.PType (pat, _) -> helper current_condition current_gen_expr pat
  in
  helper (Simple_ast.SEConst (Ast.CBool true)) (Simple_ast.SEVar gen_decl_name) pat
;;
...
let rec construct_case_expr gen_decl_name case =
  ...
  let rec helper current_expr current_gen_expr = function
    ...
    | Ast.PVar var_name ->
      Simple_ast.SELet (Ast.Nonrecursive, var_name, current_gen_expr, current_expr)
    | Ast.PTuple pat_lst ->
      let new_expr, _ =
        List.fold_left
          (fun acc pat ->
      ...
      new_expr
    | Ast.PCons (pat1, pat2) ->
      let new_expr = helper current_expr (gen_list_getter_head current_gen_expr) pat1 in
      helper new_expr (gen_list_getter_tail current_gen_expr) pat2
...
and simplify_expr expr =
  let rec helper = function
    ...
    | Ast.EFun (pat_lst, expr) ->
      ...
      let rev_new_gen_var_names, _ =
        List.fold_left
          (fun acc _ ->
            let names_lst, num = acc in
            let var_name = gen_pat_expr_name ^ string_of_int num in
            Ast.Id var_name :: names_lst, num + 1)
          ([], 0)
          pat_lst
      in
      let new_gen_var_names = List.rev rev_new_gen_var_names in
      ...
      let new_expr =
        List.fold_left2
          (fun curr_expr pat gen_var_name ->
            construct_new_fun_expr pat gen_var_name curr_expr)
          simplified_expr
          pat_lst
          new_gen_var_names
      in
      Simple_ast.SEFun (new_gen_var_names, new_expr)
    ...
    | Ast.EMatch (expr, case_lst) ->
      ...
      let rev_equality_conditions =
        List.fold_left
          (fun acc case ->
            construct_equality_condition (Ast.Id new_gen_decl_name) case :: acc)
          []
          case_lst
      in
      ...
      let rev_cases_expr =
        List.fold_left
          (fun acc case -> construct_case_expr (Ast.Id new_gen_decl_name) case :: acc)
          []
          case_lst
      in
      ...
      Simple_ast.SELet
        (Ast.Nonrecursive, Ast.Id new_gen_decl_name, simplify_expr expr, full_expr)
...
and construct_new_nonrec_value_decl value_binding gen_decl_name =
  ...
  let rec helper acc curr_expr = function
    | Ast.PAny -> acc
    | Ast.PConst _ -> acc
    | Ast.PVar var_id -> (var_id, curr_expr) :: acc
    | Ast.PTuple tup_lst ->
      let new_acc, _ =
        List.fold_left
          (fun acc elm ->
      ...
      let rev_gen_decl_lst, rev_gen_decl_names, _ =
        List.fold_left
          (fun acc value_binding ->
            ...
            let new_gen_decl =
              Simple_ast.SSILet
                (Ast.Nonrecursive, [ Ast.Id new_gen_decl_name, simplify_expr expr ])
            in
          ...
          value_binding_lst
      in
      let gen_decl_lst = List.rev rev_gen_decl_lst in
        ...
        List.fold_left2
          (fun acc value_binding gen_decl_name ->
            ...
            let new_value_decl =
              List.fold_left
                (fun acc value_binding ->
                  Simple_ast.SSILet (Ast.Nonrecursive, [ value_binding ]) :: acc)
                []
                rev_new_value_bindings
            in
            List.append new_value_decl acc)
          []
          value_binding_lst
          gen_decl_names
      in
      List.append gen_decl_lst updated_value_bindings
  ...
  let rev_new_structure =
    List.fold_left
      (fun acc structure_item ->
        ...
        List.append new_structure_items acc)
      []
      structure
  in
