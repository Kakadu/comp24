...
let update_pattern_vars varMap env pat = 
  ...
  | Ast.PTuple pat_lst -> 
    let* updated_varMap, updated_env, rev_new_pat_lst = 
      ...
      (fun acc pat -> 
        ...
        let* updated_varMap, updated_env, updated_pat = helper curr_varMap curr_env pat in 
  ...
  | Ast.PCons (pat1, pat2) -> 
    ...
    let* updated_varMap, updated_env, updated_pat2 = helper updated_varMap updated_env pat2 in
...
and convert_expr varMap env expr =
  let rec helper varMap env = function
      ...
      let* rev_new_pat_lst, updated_varMap, env =
        List.fold_left
          (fun acc pat ->
            ...
            let* updated_varMap, updated_env, new_pattern = update_pattern_vars curr_varMap curr_env pat in 
    ...
    | Ast.ELet (Ast.Nonrecursive, value_binding, expr2) ->
      ...
      let* updated_varMap, updated_env, new_pattern = update_pattern_vars varMap env old_pattern in 
      let new_value_binding = (new_pattern, expr1) in 
      let* new_value_binding, updated_env = convert_value_binding_expr varMap updated_env new_value_binding in
    ...
    | Ast.ELet (Ast.Recursive, value_binding, expr2) ->
      ...
      let* updated_varMap, updated_env, new_pattern = update_pattern_vars varMap env old_pattern in 
      let new_value_binding = (new_pattern, expr1) in
      let* new_value_binding, updated_env = convert_value_binding_expr updated_varMap updated_env new_value_binding in
      let* new_expr2, updated_env = helper updated_varMap updated_env expr2 in
    ...
    | Ast.EMatch (expr, case_lst) -> 
      ...
      let* rev_new_case_lst, updated_env = List.fold_left 
      (fun acc case -> 
        let* curr_new_case_lst, curr_env = acc in 
        let old_pattern, old_expr = case in 
        let* updated_varMap, updated_env, new_pattern = update_pattern_vars varMap curr_env old_pattern in 
...
let update_value_binding_lst_vars varMap env value_binding_lst =
  List.fold_left
    (fun acc value_binding ->
      ...
      let* updated_varMap, updated_env, new_pattern = update_pattern_vars curr_varMap env old_pattern in 
      return ((new_pattern, expr) :: value_bindings_with_new_vars, updated_varMap, updated_env))
    (return ([], varMap, env))
    value_binding_lst
;;
...
let run_alpha_conversion (structure : Ast.structure) =
