...
let additional_std_lst = [
  ...
  "#gen_tuple_getter#", Scheme(VarSet.of_list [-1; -2], tprim_int @-> type_var (-1) @-> type_var (-2));
  "#gen_list_getter_head#", Scheme(VarSet.singleton (-1), tlist (type_var (-1)) @-> type_var (-1));
  "#gen_list_getter_tail#", Scheme(VarSet.singleton (-1), tlist (type_var (-1)) @-> tlist (type_var (-1)));
  "#gen_list_getter_length#", Scheme(VarSet.singleton (-1), tlist (type_var (-1)) @-> tprim_int);
]
