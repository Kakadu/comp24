...
let parse_and_print s = Parser.structure_from_string s |> print_result Ast_pretty_printer.print_structure
...
let%expect_test "" =
 parse_and_closure_result {| fun x -> fun y -> fun z -> z y x |};
  [%expect
    {| (fun x -> ((fun x y -> (((fun y x z -> ((z y) x)) y) x)) x)) |}]
;;
...
let%expect_test "" =
 parse_and_closure_result {| let a = fun x -> let b = x in b;;|};
  [%expect
    {| let a x = let b x = x in (b x);; |}]
;;


let%expect_test "" =
 parse_and_print {| let gen seed1 seed2 = 
...
let%expect_test "" =
 parse_and_closure_result {| let gen seed1 seed2 = 
  let gen n = n * seed2 + seed1 * 42 in
  gen 0 :: [gen 1; gen 2; gen 3]|};
  [%expect
...
let%expect_test "" =
 parse_and_closure_result {| let fac n =
  let rec fack n k =
  if n <= 1 then k 1
  else fack (n - 1) (fun m -> k (m * n))
  in
...
let%expect_test "" =
 parse_and_closure_result {| let main x = 
