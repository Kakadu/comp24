...
int64_t mlrt_create_empty_closure(int64_t fun, int64_t args_num) {
    ...
    box_t* closure_box = create_box_t(sizeof(box_header_t) + sizeof(0x18));
    closure_box->header.tag = T_CLOSURE;

    closure_t* clos = &closure_box->values;
    clos->fun = fun;
    clos->args_applied = 0;
...
int64_t create_closure_by_src(box_t* src_box, int64_t new_args_num, va_list new_args) {
    ...
    closure_t* src_clos = &src_box->values;
...
int64_t call_closure(box_t* closure_box, int64_t new_args_num, va_list new_args) {
    ...
    closure_t* closure = &closure_box->values;
    ...
    for (int i = 0; i < args_count; ++i) {
        ...
        if (i < closure->args_applied)
            ...
            args[i] = va_arg(new_args, int64_t);
    }

    int64_t res = 0;

    if (ffi_prep_cif(&cif, FFI_DEFAULT_ABI, new_args_num, &ffi_type_sint64, arg_types) == FFI_OK) {
        ffi_call(&cif, closure->fun, &res, args);
    } else {
...
int64_t apply_args_to_closure(box_t* closure_box, int64_t new_args_num, va_list new_args) {
    ...
    if (args_num_until_apply <= new_args_num) {
        ...
        if (new_args_num == 0)
            ...
            return apply_args_to_closure(closure_box, new_args_num, new_args);
    } else {
...
int64_t mlrt_apply_args_to_closure(int64_t closure_box, int64_t new_args_num, ...) {
    ...
    int64_t res = apply_args_to_closure(closure_box, new_args_num, new_args);
    va_end(new_args);
    return res;
}
